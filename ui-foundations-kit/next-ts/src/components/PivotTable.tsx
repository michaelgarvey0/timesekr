...
function defaultCellFormatter(value: number) {
  return value
}
export type PivotTableParams<T extends object> = {
  data: T[]
  options: PivotOptions<T>
  apiRef?: React.MutableRefObject<GridApi>
  cellFormatter?: (value: number) => string | number
  onCellHover?: ({
    column,
    row,
    value,
  ...
  tableSortMethod?: (a: string, b: string) => number
...
export function PivotTable<T extends object>({
  ...
  const columns: GridColDef[] = [
    {
      ...
      minWidth: 180,
      headerName: `${getFieldLabel(String(rowField))} / ${getFieldLabel(
        String(columnField)
      )}`,
      renderCell: (params: GridRenderCellParams) => {
        ...
        return (
          <>
            {renderRowHeader ? renderRowHeader(String(params.id)) : params.id}
          </>
        )
      },
    },
      ...
      }),
    {
      ...
      renderCell: (params: GridRenderCellParams) => {
        ...
        if (rowId === "Total") {
          const overallTotal = Object.values(pivotedData.data).reduce(
            (acc, curr) => {
              const rowTotal = Object.values(curr).reduce(
  ...
  const rows = Array.from(pivotedData.rowSet).map(row => {
    return {
      id: row,
      total_value:
        Object.values(pivotedData?.data?.[row]).reduce(
          (acc, curr) => acc + Number(curr),
          0
        ) || 0,
      [row]: row,
