    ...
    return (<Stack spacing={2}>
      ...
      <Stack direction="column" divider={<Divider flexItem/>} spacing={2} sx={{ my: 2 }}>
        <div>
          ...
          <Typography gutterBottom sx={{ color: 'gray.700' }}>
            {addresses.join(', ')}
          </Typography>
        </div>
        <div>
          ...
          <Grid container>
            {payments.map((payment) => (<React.Fragment key={payment.name}>
                <Stack direction="row" spacing={1} useFlexGap sx={{ width: '100%', mb: 1 }}>
                  <Typography sx={{ color: 'gray.700' }}>{payment.name}</Typography>
